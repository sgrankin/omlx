...
| 7 | Forced-sync per-section timer to identify expensive GPU regions | `tools/section_time_gemma4.py` (measurement, not optimization) | - | - | - | - | - | INSIGHT (not perf change): MoE experts (~420µs/call real GPU work) is the heaviest section. q/k/v_proj are ~50µs. SDPA is ~15µs. The model is GPU-compute-bound at batch=1; 23ms/token floor is realized at ~98% efficiency. Bigger wins need algorithm-level changes (TurboQuant, spec decode). |
